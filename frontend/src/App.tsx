...
function App() {
  ...
  useEffect(() => {
    if (!samples.length) {
      axios.get("http://localhost:8888/samples").then((res) => {
        setSamples(res.data);
      });
    }
  }, []);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const assetsResponse = await axios.get(
      "https://localhost:8888/create-story?url=" + encodeURIComponent(url)
    );

    const id = await assetsResponse.data;
    setLoadingMessage("Generating your video...");
    const videoResponse = await axios.get(
      "https://localhost:8888/build-video?id=" + id
    );
    setLoadingMessage("");

    window.location.href = "https://localhost:8888/" + videoResponse.data;
  }
      ...
      <main className="max-w-2xl mx-auto flex gap-16 px-4">
        <div className="py-8 flex flex-col justify-center">
          <h1 className="text-4xl font-bold uppercase mb-4">
            <span className="text-5xl">Your URL to Video</span>
            <br />
